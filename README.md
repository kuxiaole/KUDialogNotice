...
构建产物位于 `target/KUDialogNotice-1.0.1.jar`。
...
MariaDB 会自动创建 `<prefix>players`、`<prefix>rules` 和 `<prefix>changelog_seen`。Redis 只作为共享读取缓存，MariaDB 始终是最终事实来源。缓存 key 会自动加入由 MariaDB JDBC URL 与表前缀生成的命名空间，切换权威数据库时不会误用旧库缓存。需要 TLS 时可使用 `rediss://` URI。
...
- Redis 暂时不可用时，普通读取和写入会回退到 MariaDB，并在日志中给出一次警告；MariaDB 不可由 Redis 替代。

## 跨服与版本号

`config.yml` 中的 `server-id` 是更新日志流标识，只允许小写字母、数字、点、下划线和连字符，最长 64 个字符。

```yaml
server-id: "survival"
```

- 不同玩法子服使用不同值，例如 `survival`、`skyblock`，这样各自独立维护更新日志。
- 同一个逻辑子服的多个实例使用相同值。
- 服规同意记录不区分 `server-id`，因此全网共享。
- 更新日志已读记录按“玩家 UUID + `server-id`”共享。
- YAML 公告内容本身不会跨服同步；需按运营方式手工部署到各子服。
...
每次修改需要重新展示的更新内容时，都必须把 `revision` 严格递增。不要回退或复用旧值；仅修改 `version-label` 或正文而不增加 `revision`，已读玩家不会再次看到。`version-label` 只负责展示，可以使用日期或自定义版本名。
